...
import {
  BuildingIcon, CreditCardIcon, CheckCircleIcon,
  ClockIcon, AlertCircleIcon, XCircleIcon, TrendingUpIcon,
} from "lucide-react";
  ...
  useEffect(() => {
    fetch("/api/admin/restaurants")
      ...
      .then((data) => {
        ...
        else setRestaurants(data.value ?? data);
      })
  ...
  if (error) return (
    <div className="p-8">
      <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-6">
        ...
        <p className="text-red-500/70 text-sm mt-1 font-mono">{error}</p>
      </div>
    </div>
  );
  ...
  if (!restaurants) return (
    <div className="p-8 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[...Array(4)].map((_, i) => <div key={i} className="h-24 bg-neutral-800 rounded-2xl animate-pulse" />)}
      </div>
    </div>
  );
  ...
  return (
    <div className="p-8 space-y-8">
      <div>
        ...
        <p className="text-neutral-500 text-sm mt-1">Painel de controle — Foodpronto · {total} restaurantes</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard icon={<BuildingIcon className="w-5 h-5 text-blue-400" />}   label="Total"           value={String(total)}   bg="bg-blue-400/10" />
        <KpiCard icon={<TrendingUpIcon className="w-5 h-5 text-green-400" />} label="MRR estimado"    value={`R$ ${mrrValue}`} bg="bg-green-400/10" />
        <KpiCard icon={<CheckCircleIcon className="w-5 h-5 text-green-400" />} label="Ativos"         value={String(active)}  bg="bg-green-400/10" />
        <KpiCard icon={<ClockIcon className="w-5 h-5 text-blue-400" />}      label="Em trial"         value={String(trial)}   bg="bg-blue-400/10" />
      </div>
      ...
      <div className="flex gap-3">
        <Link href="/admin/restaurantes" className="flex items-center gap-2 bg-[#E24B4A] hover:bg-[#c93f3e] text-white px-5 py-2.5 rounded-full text-sm font-semibold transition-colors">
          <BuildingIcon className="w-4 h-4" /> Ver restaurantes
        </Link>
        <Link href="/admin/assinaturas" className="flex items-center gap-2 border border-neutral-700 hover:border-neutral-500 text-neutral-300 px-5 py-2.5 rounded-full text-sm font-semibold transition-colors">
          <CreditCardIcon className="w-4 h-4" /> Assinaturas
        </Link>
      </div>
    </div>
  );
